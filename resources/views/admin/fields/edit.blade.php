<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar cancha: {{ $field->name }}</h2>
    </x-slot>
            ...
            <form method="POST" action="{{ route('admin.fields.update', $field) }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="venue_id"
                        ...
                        @foreach($venues as $venue)
                            <option value="{{ $venue->id }}" {{ old('venue_id', $field->venue_id) == $venue->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="sport_type_id"
                        ...
                        @foreach($sportTypes as $sport)
                            <option value="{{ $sport->id }}" {{ old('sport_type_id', $field->sport_type_id) == $sport->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $field->description) }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="surface"
                            class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="synthetic" {{ old('surface', $field->surface) == 'synthetic' ? 'selected' : '' }}>Sintética</option>
                        <option value="grass" {{ old('surface', $field->surface) == 'grass' ? 'selected' : '' }}>Césped natural</option>
                        <option value="cement" {{ old('surface', $field->surface) == 'cement' ? 'selected' : '' }}>Cemento</option>
                    </select>
                </div>

                <div class="mb-6 flex items-center gap-2">
                    <input type="checkbox" name="active" id="active" value="1"
                           {{ old('active', $field->active) ? 'checked' : '' }}>
